...
class StripeWHHandler:
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        try:
            ...
            logger.debug(f'Bag: {bag}, Save Info: {save_info}')

            billing_details = intent.charges.data[0].billing_details
            shipping_details = intent.shipping
            grand_total = round(intent.charges.data[0].amount / 100, 2)
            logger.debug(f'Billing Details: {billing_details}, Shipping Details: {shipping_details}, Grand Total: {grand_total}')
            ...
            if username != 'AnonymousUser':
                try:
                    ...
                    if save_info:
                        ...
                        profile.save()
                    logger.debug(f'UserProfile: {profile}')
                except UserProfile.DoesNotExist:
                    logger.error(f'UserProfile does not exist for username: {username}')
            ...
            while attempt <= 5:
                try:
                    ...
                    attempt += 1
                    time.sleep(1)
                    logger.debug(f'Order not found, attempt {attempt}')
            ...
            logger.error(f'Unhandled exception in handle_payment_intent_succeeded: {e}', exc_info=True)
            return HttpResponse(
                content=f'Webhook received: {event["type"]} | ERROR: {e}',
                status=500)
